...
- Запуске бота
- Отписках и блокировках пользователей
- Разблокировках администратором
- Новых подписках (в том числе после разблокировки)
- Всех изменениях статусов участников
- Ошибках

Примеры записей в логе:
```
# Пользователь отписался и был заблокирован
2025-10-25 10:30:45 - __main__ - INFO - Пользователь отписался и заблокирован: Иван Иванов (ID: 123456789, Username: @ivan) | member (подписан) -> left (отписался)

# Администратор разблокировал пользователя
2025-10-25 10:35:20 - __main__ - INFO - ✓ Администратор разблокировал пользователя: Иван Иванов (ID: 123456789, Username: @ivan) | kicked -> left

# Пользователь подписался (возможно после разблокировки)
2025-10-25 10:36:15 - __main__ - INFO - → Пользователь подписался на канал: Иван Иванов (ID: 123456789, Username: @ivan) | left -> member
```
...
### Поведение при разблокировке администратором

**Важно:** Бот корректно обрабатывает случаи ручной разблокировки:

1. **Админ разблокировал пользователя** (kicked → left):
   - Бот НЕ будет блокировать его повторно
   - В логах появится: `✓ Администратор разблокировал пользователя`

2. **Пользователь подписался после разблокировки** (left → member):
   - Бот НЕ будет блокировать его за подписку
   - В логах появится: `→ Пользователь подписался на канал`

3. **Пользователь снова отписался** (member → left):
   - Бот заблокирует его снова (это нормальное поведение)
   - В логах появится: `Пользователь отписался и заблокирован`

Таким образом, вы можете давать пользователям второй шанс, разблокируя их вручную. Бот не будет мешать.
